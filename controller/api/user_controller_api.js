...
exports.deleteUser = async (req, res, next) => {
  ...
  } else res.status(code.BAD_REQUEST).json({ status: "Xoá không thành công" });
};
...
exports.logInUser = async (req, res, next) => {
  ...
  if (data != null) res.status(200).json({ status: "Đăng nhập thành công" });
  else res.status(code.FORBIDDEN).json({ status: "Đăng nhập thất bại" });
};
